...
if (!specFolders.length) {
  help();
}

if (autotest) {
  for (var i = 0; i < specFolders.length; i++) {
    folder = specFolders[i];
    require('./autotest').start(folder,
...
if(useHelpers){
  for (var i = 0; i < specFolders.length; i++) {
    folder = specFolders[i];
    jasmine.loadHelpersInFolder(folder,
...
jasmine.executeSpecsInFolders(specFolders,
                             onComplete,
                             isVerbose,
                             showColors,
                             teamcity,
